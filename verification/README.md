...
[results.json](results.json) records the upstream revision, dependency manifest, tool revisions, artifact hashes and accepted comparison targets. The `867` and `769` directories contain the exact challenge, solution, configuration and successful logs from the recorded proof replays. Paths and unrelated experiment records are excluded from this package.
...
This clones the recorded upstream revision under `.build/formal-conjectures`, obtains its pinned dependency cache, applies both patches, and runs `lake --wfail build` on the two problem modules and the regression. It does not build every conjecture. Dependency downloads can be large. If the exact checkout already has trusted dependencies installed, use `--skip-cache`. `--checkout PATH` selects an existing checkout at the recorded revision; unrelated tracked edits are refused.
